"""Materialize immutable run instructions for one Codex CLI turn."""
...
def materialize_instruction_snapshot(
    ...
    """Expose frozen Skills to Codex for one turn and remove them afterward."""
    ...
    try:
        if skills:
            for root in (checkout_path / '.agents', skills_root):
                if not root.exists():
                    root.mkdir()
            ...
            for name, description, content, _, _ in skills:
                ...
                if target.exists() or target.is_symlink():
                    raise ValueError(
                        f'Codex Skill path already exists in the checkout: {target}'
                    )
                target.mkdir()
                created_skills.append(target)
                (target / 'SKILL.md').write_text(
                    _skill_document(name, description, content),
                    encoding='utf-8',
                )
        ...
        for root in reversed(created_roots):
            try:
                ...
                # Preserve unrelated files that Codex or the checkout may have added.
                pass
...
def _skill_name(skill: Mapping[str, object]) -> str:
...
def _skill_document(name: str, description: str, content: str) -> str:
    """Render one valid Codex SKILL.md file with YAML frontmatter."""
